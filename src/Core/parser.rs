...
pub enum ParseError { Malformed, Unsupported }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TransportMetadata {
    Tcp { source_port: u16, destination_port: u16, data_offset: u8, flags: u16 },
    Udp { source_port: u16, destination_port: u16, length: u16 },
}
...
pub fn parse(bytes: &[u8]) -> Result<Metadata, ParseError> {
    ...
    let (header_length, total_length, protocol, source, destination) = match version {
        ...
        6 => {
            if bytes.len() < 40 { return Err(ParseError::Malformed); }
            let payload = usize::from(be16(bytes, 4));
            if payload == 0 && bytes.len() > 40 { return Err(ParseError::Unsupported); }
            let total = 40 + payload;
            if total != bytes.len() { return Err(ParseError::Malformed); }
            let mut source = [0u8; 16];
            let mut destination = [0u8; 16];
            source.copy_from_slice(&bytes[8..24]);
            destination.copy_from_slice(&bytes[24..40]);
            (40, total, bytes[6], Ipv6Addr::from(source).into(), Ipv6Addr::from(destination).into())
        }
    ...
    let transport = match protocol {
        6 => {
            if payload.len() < 20 { return Err(ParseError::Malformed); }
            let offset = payload[12] >> 4;
            if offset < 5 || usize::from(offset) * 4 > payload.len() {
                return Err(ParseError::Malformed);
            }
            TransportMetadata::Tcp {
                source_port: be16(payload, 0), destination_port: be16(payload, 2),
                data_offset: offset, flags: (u16::from(payload[12] & 1) << 8) | u16::from(payload[13]),
            }
        }
        17 => {
            if payload.len() < 8 { return Err(ParseError::Malformed); }
            let length = be16(payload, 4);
            if length < 8 || usize::from(length) != payload.len() {
                return Err(ParseError::Malformed);
            }
            TransportMetadata::Udp { source_port: be16(payload, 0), destination_port: be16(payload, 2), length }
        }
        _ => return Err(ParseError::Unsupported),
    };
    Ok(Metadata {
        version, header_length, total_length, payload_length: total_length - header_length,
        protocol, source, destination, transport,
    })
}
