...
impl Phase2Config {
    ...
    fn validate_fields(&self) -> Result<(), CaptureError> {
        if self.destination_ips.len() > 32
            || self.tcp_ports.len() > 32
            || self.udp_ports.len() > 32
            || self.tcp_ports.contains(&0)
            || self.udp_ports.contains(&0)
        {
            return Err(invalid());
        }
        for ip in &self.destination_ips {
            let unsuitable = ip.is_unspecified()
                ...
                || match ip {
                    ...
                    IpAddr::V6(v6) => {
                        v6.to_ipv4().is_some() || (v6.segments()[0] & 0xffc0 == 0xfe80)
                    }
                };
            if unsuitable {
                return Err(invalid());
    ...
    pub fn filter(&self) -> Result<String, CaptureError> {
        ...
        for (protocol, ports) in [("tcp", &self.tcp_ports), ("udp", &self.udp_ports)] {
            let ports: BTreeSet<_> = ports
                .iter()
                .map(|p| format!("{protocol}.DstPort == {p}"))
                .collect();
            if !ports.is_empty() {
                transport.push(format!(
                    "({protocol} and ({}))",
                    ports.into_iter().collect::<Vec<_>>().join(" or ")
                ));
            }
        }
        Ok(format!(
            "outbound and !loopback and !impostor and ({}) and ({})",
            ips.into_iter().collect::<Vec<_>>().join(" or "),
            transport.join(" or ")
        ))
    }
}
